...
namespace NSAP_ODK.Entities
{
    ...
    public static class GenerateCSV
    {
        ...
        public static async Task<int> GenerateAll()
        {
            ...
            int result = await GenerateCSV.GenerateEffortSpecCSV();
            FilesCount++;
            ...
            {
                result += await GenerateCSV.GenerateItemsetsCSVEX();
            }
            else
            {
                result += await GenerateCSV.GenerateItemsetsCSV();
                FilesCount++;
            }


            _fileName = $"{_folderSaveLocation}\\regionFMA_fmaID.csv";
            result += await GenerateCSV.GenerateFMAIDsCSV();
            FilesCount++;

            _fileName = $"{_folderSaveLocation}\\majorgrid_fma.csv";
            result += await GenerateCSV.GenerateMajorGridsInFMA();
            FilesCount++;

            _fileName = $"{_folderSaveLocation}\\inlandgrids.csv";
            result += await GenerateCSV.GenerateInlandGridCSV();
            FilesCount++;

            _fileName = $"{_folderSaveLocation}\\sp.csv";
            result += await GenerateCSV.GenerateMultiSpeciesCSV();


            _fileName = $"{_folderSaveLocation}\\size_measure.csv";
            result += await GenerateCSV.GenerateSizeTypesCSV();
            FilesCount++;
            ...
            result += await GenerateCSV.GenerateFishingVesselNamesCSV(filePaths);
